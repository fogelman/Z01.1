	...
	public void writePushPop(Parser.CommandType command, String segment, Integer index) {
		...
		if (command == Parser.CommandType.C_POP) {
			...
			if (segment.equals("constant")) {
				...
				commands.add("leaw (%A), %A"); // pegou o valor que o stack pointer aponta
				commands.add("decw %A"); // subtraiu 1 do stackpointer e salvou em A
				commands.add("movw (%A), %S");
				commands.add("leaw $LCL,%A");
				commands.add("movw (%A), %D"); // pegou o valor que o local aponta e coloca em D
				if (index > 0) { // se o index for maior que zero => fazer a soma do valor
					commands.add("leaw $" + index + ",%A");
					commands.add("addw %A, %D, %A"); // Atualiza o valor em A
				}

				commands.add("movw %S,(%A)");// atualiza o local
			...
			if (segment.equals("constant")) {
				...
				commands.add("movw (%A),%D"); // pega o valor e coloca em D
